...
| `--no-build` | Skip the install + build of the generated addon (on by default — an unbuilt addon exports a `dist/` that does not exist yet, so every `ref()` into it is dead at runtime) |
...
CLI commands follow the same shape, but the `cli` leaf is retired for addons —
`#pikku/addon/cli` is never generated. Import `defineCLICommands` from
`@pikku/core/cli` directly; the consuming app mounts the result with `refCLI`.
`wireCLI` registers a program with the app's own CLI, which is not something an
addon reaches.
...
```bash
npx pikku all && tsc && npx pikku dist && npm publish
```
