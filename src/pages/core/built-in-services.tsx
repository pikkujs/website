...
function ProviderBadge({ name }: { name: string }) {
  return (
    <span className="inline-block text-[11px] font-mono font-medium bg-white/[0.06] text-white/65 border border-white/12 rounded px-2 py-0.5">
      {name}
    </span>
  );
}
  ...
  return (
    <div className="wire-hero-builtins w-full relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none" aria-hidden>
        ...
        <div className="absolute left-[10%] top-[30%] w-56 h-56 rounded-full bg-white/[0.04] blur-[80px]" />
      </div>

      <header className="relative max-w-screen-xl mx-auto w-full pt-12 pb-10 lg:pt-16 lg:pb-14 px-6 ">
          ...
          <div key={group.title}>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {group.services.map((service) => {
                ...
                const content = (
                  <div
                    className={`group relative bg-[#0d0d0d] border border-neutral-800 rounded-xl p-6 transition-all ${service.docUrl ? 'hover:border-white/15 hover:bg-white/[0.02]' : ''}`}
                  >
                    <div className="flex items-start gap-4 mb-4">
                      <div className="flex-shrink-0 w-11 h-11 rounded-xl bg-white/[0.06] border border-white/12 flex items-center justify-center">
                    ...
                    {service.docUrl && (
                      <span className="absolute bottom-4 right-5 text-xs font-semibold text-primary/50 group-hover:text-primary transition-colors">
                        Docs &rarr;
                      </span>
                    )}
                  </div>
      ...
      <div className="max-w-screen-xl mx-auto px-6 relative">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <Link
            ...
            className="bg-primary text-black hover:bg-primary px-8 py-3 rounded-lg font-semibold text-lg transition-all hover:scale-105 shadow-lg shadow-orange-500/20"
          >
            Services Docs
          </Link>
          <Link
